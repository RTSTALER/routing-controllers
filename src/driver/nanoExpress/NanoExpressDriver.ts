...
import {NanoExpressMiddlewareInterface} from "./NanoExpressMiddlewareInterface";
import {NanoExpressErrorMiddlewareInterface} from "./NanoExpressErrorMiddlewareInterface";
...
import { nanoActionMap } from "./NanoExpressActionMap";
 ...
 */
export class NanoExpressDriver extends BaseDriver {

    // -------------------------------------------------------------------------
    // Constructor
    // -------------------------------------------------------------------------

    constructor(public nanoExpress?: any) {
        super();
        this.loadExpress();
        this.app = this.nanoExpress;
    }
    ...
    initialize() {
        if (this.cors) {
            ...
            if (this.cors === true) {
                this.nanoExpress.use(cors());
            } else {
                this.nanoExpress.use(cors(this.cors));
            }
        }
    }
    ...
    registerMiddleware(middleware: MiddlewareMetadata): void {
        ...
        if ((middleware.instance as NanoExpressErrorMiddlewareInterface).error) {
            middlewareWrapper = (error: any, request: any, response: any, next: (err?: any) => any) => {
                (middleware.instance as NanoExpressErrorMiddlewareInterface).error(error, request, response, next);
            };
        }

        // if its a regular middleware then register it as express middleware
        else if ((middleware.instance as NanoExpressMiddlewareInterface).use) {
            middlewareWrapper = (request: any, response: any, next: (err: any) => any) => {
                try {
                    const useResult = (middleware.instance as NanoExpressMiddlewareInterface).use(request, response, next);
        ...
        if (middlewareWrapper) {
            ...
            this.nanoExpress.use(middlewareWrapper);
        }
    }
    ...
    registerAction(actionMetadata: ActionMetadata, executeCallback: (options: Action) => any): void {
        ...
        let action = actionMetadata.type.toLowerCase();
        for (const [key, value] of Object.entries(nanoActionMap)) {
            if (action === key.toLowerCase()) {
                action = value.toLowerCase();
            }
        }
        this.nanoExpress[action.toLowerCase()](...[
            route,
            routeGuard,
            ...beforeMiddlewares,
            ...defaultMiddlewares,
        ...
        uses.forEach(use => {
            if (use.middleware.prototype && use.middleware.prototype.use) { // if this is function instance of MiddlewareInterface
                middlewareFunctions.push((request: any, response: any, next: (err: any) => any) => {
                    try {
                        const useResult = (getFromContainer(use.middleware) as NanoExpressMiddlewareInterface).use(request, response, next);
                        if (isPromiseLike(useResult)) {
                            useResult.catch((error: any) => {
                                this.handleError(error, undefined, {request, response, next});
                                return error;
                ...
                middlewareFunctions.push(function (error: any, request: any, response: any, next: (err: any) => any) {
                    return (getFromContainer(use.middleware) as NanoExpressErrorMiddlewareInterface).error(error, request, response, next);
                });
    ...
    protected loadExpress() {
        if (require) {
            if (!this.nanoExpress) {
                try {
                    this.nanoExpress = require("nanoexpress-pro")();
                } catch (e) {
